...
The job submission body accepts an optional `parameters` field for parametric circuits (OpenQASM 3.0 `input float[64]` declarations). This enables compile-once, run-many execution: Afana compiles a parametric program once, and the caller binds concrete values at submission time.
...
HAL drivers that do not support parametric circuits MUST return an error when `parameters` is non-empty.
